...
window.app = {
    ...
    // 渲染下拉選單
    renderTemplateDropdown(selectedId = '') {
        const select = document.getElementById('templateSelect');
        if (!select) return;

        let html = `<option value="" disabled ${!selectedId ? 'selected' : ''}>-- 選擇現有模板 --</option>`;
        
        customTemplates.forEach(t => {
            html += `<option value="${t.id}" ${t.id === selectedId ? 'selected' : ''}>${t.name}</option>`;
        });

        html += `<option value="ADD_NEW" class="text-emerald-600 font-bold">+ 新增模板</option>`;
    ...
    handleTemplateChange(val) {
        if (val === 'ADD_NEW') {
            ...
            document.getElementById('templateSelect').value = ""; // 重置選單
            return;
        }
    ...
    saveNewTemplate() {
        ...
        const newTpl = { id: newId, name, p, c, f, w };
        
        customTemplates.push(newTpl);
        
        // 儲存並更新介面
        storage.save(config, historyData, commonFoods, customTemplates);
        this.renderTemplateDropdown(newId);
        
        // 自動套用數值到目標輸入框
        document.getElementById('targetProtein').value = p;
        document.getElementById('targetCarbs').value = c;
        document.getElementById('targetFat').value = f;
        document.getElementById('targetWater').value = w;
        this.updateTargets();

        // UI 跳轉
        ui.closeModal('newTemplateModal');
        ui.openModal('goalModal');
        ui.showMessage(`模板 "${name}" 建立成功`);
